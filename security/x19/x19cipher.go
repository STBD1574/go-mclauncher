...
func X19HttpEncrypt(bodyIn []byte) ([]byte, error) {
	...
	copy(body, bodyIn)
	randFill := []byte(security.RandomString(0x10))
	for i := 0; i < len(randFill); i++ {
		body[i+len(bodyIn)] = randFill[i]
	}

	keyQuery := byte(rand.Intn(15))<<4 | 2
	initVector := []byte(security.RandomString(0x10))
	encrypted, err := security.AesCBCEncrypt(X19PickKey(keyQuery), body, initVector)
	if err != nil {
		return nil, err
	}
...
func X19HttpDecrypt(body []byte) ([]byte, error) {
	...
	result, err := security.AesCBCDecrypt(X19PickKey(body[len(body)-1]), body[16:len(body)-1], body[:16], false)
	if err != nil {
		return nil, err
	}

	// Remove padding (it is not PKCS7 paddinng)
	scissor := 0
